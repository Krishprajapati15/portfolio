...
export default function Game({ turnsLeft, setTurnsLeft }: props) {
  ...
  const defaultGameTimerValue = 60 * 0.5;
    ...
    <div className="relative flex flex-col bg-muted rounded-md p-2 size-64">
      {isStart && (
        <>
          <Button
            ...
            variant="ghost"
            size="icon"
            className="absolute top-2 right-2 size-fit text-gray-500"
          >
            <X size={16} />
          </Button>
          <div className="w-full text-center">
            {`${Math.floor(gameTimer / 60)}`.padStart(2, "0")}:
            {`${gameTimer % 60}`.padStart(2, "0")}
          </div>
      ...
      {!isStart && (
        <>
          <div
            className={cn(
              "top-0 left-0 absolute size-full flex flex-col gap-1 items-center justify-center backdrop-blur-md z-[2]",
              isStart && "hidden"
            )}
          >
            <Button
